...
from mwlib.templ.magics import magic_words, MagicResolver
...
def replace_tags(txt):
    #patt = re.compile(r"<!--.*?-->|<(nowiki|math|gallery|source|pre|timeline|poem)>.*?</\1>", flags=re.DOTALL|re.UNICODE|re.IGNORECASE)
...
class Summary:
    ...
    def print_summary(this):
        print "Started: ".rjust(30) + this.starttime
        print "Done: ".rjust(30) + time.strftime("%a, %d %b %Y %H:%M:%S", time.localtime()) 
        print "Aricles: ".rjust(30) + str(this.articles)
        print "Redirects: ".rjust(30) + str(this.redirects)
        print "Missing templates: ".rjust(30) + str(this.missing_templates)
        print "Articles with parse errors: ".rjust(30) + str(this.parseerrors)
        print "Other errors: ".rjust(30) + str(this.errors)
